...
use Modules\WebCompany\Repository\Page\PageFacade as Page;
use Modules\WebCompany\Repository\Service\ServiceFacade as Service;
use Modules\WebCompany\Repository\PageTranslation\PageTranslationFacade as PageTranslation;
...
class WebCompanyController extends Controller
{
    ...
    public function home(){
        ...
        return view('webcompany::home',compact('page'))->with("data",$this->data);
    }
    public function about_us(){
        $page=Page::find(4,['*'],[],['content']);//Get About_us Page Data
        return view('webcompany::about_us',compact('page'))->with("data",$this->data);
    }
    public function services(){
        $page=Page::find(3,['*'],[],['content']);//Get Service Page Data
        $services=Service::index(['*'],[],[],0);//Get All services
        return view('webcompany::services',compact('page','services'))->with("data",$this->data);
    }
    public function login(){
        if (Auth::user()) {return redirect('/');}
        $page=Page::find(6,['*'],[],['content']);//Get Login Page Data
        return view('webcompany::login',compact('page'))->with("data",$this->data);
    }
    ...
    public function service($slug){
        ...
        return view('webcompany::service',compact('page','service'))->with("data",$this->data);
    }
