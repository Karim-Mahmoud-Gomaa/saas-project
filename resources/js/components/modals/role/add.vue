<template>
    <!--begin::Modal-->  
    <div class="modal fade" id="roleModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
...
export default {
    ...
    methods: {
        ...
        store(){ 
            this.form.post("roles").then(({data}) => {
                this.fetchData();
        ...
        update(){
            this.form.put("roles/"+this.form.id).then(({data}) => {
                this.fetchData();
                $('#roleModal').modal('hide');
